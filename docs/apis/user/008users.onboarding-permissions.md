...
Account setup step 2 — records whether the user allowed push notifications.
...
|---|---|
| **Method** | `POST` |
| **Path** | `/api/users/me/onboarding/permissions` |
...
|---|---|---|---|---|
| `notifications_enabled` | boolean | yes | Must be `true` or `false`. | The answer given to the device permission prompt. |
...
```bash
curl -X POST http://localhost:4000/api/users/me/onboarding/permissions \
...
- This is step 2 of 4.
- Send `false` when the user declines, so the server stops queueing push notifications for them.
- This flag does not affect in-app notifications, which are always stored and readable from `GET /api/notifications`.
