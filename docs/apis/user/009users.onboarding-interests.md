...
Account setup step 3 — saves the interest tags the user picked.
...
|---|---|
| **Method** | `POST` |
| **Path** | `/api/users/me/onboarding/interests` |
...
|---|---|---|---|---|
| `interests` | string[] | yes | 1 to 20 strings. | Interest tags, for example `gaming`, `shopping`, `travel`. |
...
```bash
curl -X POST http://localhost:4000/api/users/me/onboarding/interests \
...
- This is step 3 of 4.
- The array replaces the stored list; it is not merged.
- Tags are stored as-is in a `jsonb` column, so the client controls the vocabulary.
