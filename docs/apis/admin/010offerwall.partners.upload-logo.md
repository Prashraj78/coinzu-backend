...
Uploads an offerwall partner logo and returns its public URL. Upload first, then send the returned `url` as `logo_url` to the create or update endpoint.
...
|---|---|
| **Method** | `POST` |
| **Path** | `/api/admin/offerwall/partners/logo` |
...
|---|---|---|---|---|
| `file` | file | yes | JPEG, PNG or WebP, at most 5 MB. | The logo image to upload. |
...
|---|---|---|
| `url` | string | Public URL of the stored logo. |

```json
{
  "success": true,
  "data": {
    "url": "https://pub-3d84c195d8854a1aaf0f51c634bfa899.r2.dev/offerwall-logos/17cc263c-a51d-41d8-b0c7-05fd6a5b31e4.png"
  }
}
```
...
```bash
curl -X POST http://localhost:4000/api/admin/offerwall/partners/logo \
  -H 'Authorization: Bearer <rewardtym_admin_access_token>' \
  -F 'file=@rewardtym-logo.png'
```

## Notes

- Send the image as `multipart/form-data` under the field name `file`.
- Uploading does not attach the logo to any partner. Send the returned `url` as `logo_url` to `POST /api/admin/offerwall/partners` or `PATCH /api/admin/offerwall/partners/:id` to actually set it.
- Files are stored under `offerwall-logos/` with a generated name, so the original file name is never exposed. The old logo at a replaced URL is not deleted.
